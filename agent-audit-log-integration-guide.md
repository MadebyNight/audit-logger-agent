...
上游 Agent 应在理解用户请求后、开始业务执行前形成目标摘要，并随 `run.start` 上报。`original_request` 记录用户提出了什么，`expected_purpose` 记录 Agent 理解后要达成什么，`agent_result` 记录实际完成了什么。

- 写清目标结果、影响任务正确性的范围（如对象、业务日期、时区）以及判断完成的标准；只写与该任务有关的内容。
- 保存理解结论，不要求完整推理过程，也不以工具清单或执行步骤代替目标。
- 不机械复制原始请求，不仅加上“用户指令交互”“完成用户需求”等前缀；不在执行后倒填或改写目的以迁就结果。
- 不臆造用户未提出的需求。关键日期、范围等尚未明确时，记录待确认项；“今天”依据任务发生时已明确的业务时区解释，不能依据审计运行当天解释。
- 简单且明确的请求可能已经包含完整目标，允许语义或文本一致。文本相同本身不构成拒收条件或业务风险；审计应判断目标是否足够明确，以及实际结果是否符合请求。

例如，任务发生于北京时间 2026-09-21，原始请求为“跑一波今日日报吧～”：

| 填写示例 | 说明 |
| --- | --- |
| `用户指令交互：跑一波今日日报吧～` | 反例：仅重复原话，没有说明理解后的目标与完成标准 |
| `调用 cap.report.query 并发送消息` | 反例：只有操作步骤，没有说明要交付哪一天、什么结果 |
| `生成北京时间 2026-09-21 的经营日报，核实数据日期与覆盖范围并汇总关键经营指标；若当天数据不可用，明确说明缺失或延迟，不将历史数据作为今日日报。` | 正例：目标、日期范围和结果要求明确；实际填写须以已知业务上下文为依据 |

若实际返回的是 2026-08-18 日报，应对照请求、目的及事件证据指出日期偏差；若目的本身误写为生成 8 月 18 日日报，也不能因此认定满足用户请求。只有事件明确支持任务日期和业务时区时才能作此判断，信息不足则说明无法核实。目的空泛属于审计上下文质量问题，不等同于任务执行失败。

服务端保留上游上报的目的作为审计证据，不自动改写历史记录。上游接入实现应同步遵循上述填写规则。
