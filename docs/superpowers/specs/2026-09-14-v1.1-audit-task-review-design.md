...
> 2026-09-18 规范修订：本文是历史设计记录。当前接入规则以 `agent-audit-log-integration-guide.md` 第 5 节和 `scripts/lib/auditSpec.js` 为准：取消 compat/strict 切换，统一严格校验；Span 字段可选，`expected_purpose` 在 `run.start` 必填。下文旧模式、Span 必填和预期目的可选的描述已被取代。
...
- `requester_id`：行为发起用户的姓名，允许明文保存和展示，无需哈希或匿名化，同一用户应保持一致写法。`run.start` 必填，其他事件可继承或重复携带。
...
- **责任方在上游接入方**：根据 2026-09-22 确认的业务要求，`requester_id` 直接填写行为发起用户的姓名，允许明文保存和展示，无需哈希或匿名化，同一用户应保持一致写法。此例外仅适用于该字段中的发起人姓名，手机号、邮箱、身份证号及其他字段仍须遵守原有脱敏要求，`user_id` 和 `decision.actor_id` 的脱敏约定不变。已有脱敏标识仍可接收，历史数据不会自动转换为姓名。
- **分组影响**：同一 Agent 下按 `requester_id` 分组，同名用户会合并，姓名或写法变化会拆成不同用户；从旧标识切换到姓名也会形成新的分组。
