...
第 3 行的"最后一跳不是 `run.waiting_user`"是必需的排除条件，不是冗余表述。缺它时优先级 2 永远不可达：等待用户的 Trace 在第 60 分钟就先命中优先级 3 被封存为 `idle_timeout`，1440 分钟的阈值没有任何机会生效，用户还没来得及回话，任务就已经在列表里显示为终态。加上排除条件后，等待用户期间该 Trace 保持未封存、显示"审查中"，直到真正超时。

`waiting_user_timeout` 保留为独立的 `sealed_reason` 取值，用途是排障时区分"用户没回"和"链路断了"；它不导向任何风险等级，超时封存后按 6.3.1 正常判定（通常落到"无终止事件且无异常证据"，即 `incomplete + none`，展示为"待确认"）。
...
- `action_state` 按 6.5 的两个轴（`sealed_at`、`review_version`）派生；未封存 Trace 的行动状态为 `in_progress`，中文文案"审查中"。
...
| `waitingUserTimeoutMinutes` | 1440 | 可调初始值 | 等待用户的容忍时长；超时后按 `incomplete + none` 判"待确认"，不发告警，因此调大调小不会放大误报 |
...
- **下限由功能决定，不是由存储决定。** `unconfirmed`（"待确认"）是一个要求真人回来确认结果的行动状态，保留期必须覆盖人的响应周期，否则用户周五收到通知、周一回来点进去时证据已被清理，该状态就失去意义。跨周末需要 3 天，跨假期需要 7 天以上，**7 天是绝对下限，14 天是舒适下限**。日报里链接回去的任务同样受此约束。
...
- `run.waiting_user` 不单独导向任何风险等级：无论是否存在 `run.resume`，都回到本表按终止事件与异常证据继续判定。等待用户超时只影响 5.3 的封存时机与 `sealed_reason`，不影响结论。超时封存的 Trace 通常落到第 6 行的 `incomplete + none`，在任务列表中显示为"待确认"——用户长时间未回复自己的 Agent 是正常等待，不是异常中断，不应触发"需要人工介入"的高风险告警。
...
| --- | --- | --- | --- |
| `in_progress` | `sealed_at IS NULL` | 审查中 | 数据尚未收齐，任务可能仍在进行 |
| `unreviewed` | `sealed_at` 非空且 `review_version = 0` | 未审查 | 数据已收齐但尚未出结论；历史回填数据全部落在此状态 |
| `needs_attention` | `risk_level = high` | 需要介入 | 任务失败或中断，必须人工介入 |
| `unconfirmed` | `trace_status = incomplete` 且 `risk_level ∈ {none, low}` | 待确认 | 链路缺终止事件，但审计未判定为风险项，结果无法确认 |
...
因此任务列表该列只会出现五种文案：未封存时"审查中"，已封存未出结论时"未审查"，`risk_level=high` 时"需要介入"，`incomplete` 且未判定风险时"待确认"，其余已完成任务显示"已完成"。由于 `incomplete` 只允许搭配 `none`、`low`，不会出现"已完成但链路缺终止事件"的任务。
...
首页不展示风险等级、Agent 页只展示发起用户分组和任务行动状态、用户任务列表不出现成功/失败原始术语——这是刻意的信息分层，不是遗漏。分层依据是"不同层级回答不同问题"：

| 页面 | 回答的问题 | 因此不展示 |
| --- | --- | --- |
| 首页 | 有哪些 Agent、最近是否有活动 | 风险等级、工具、Finding、状态码 |
| Agent 页 | 谁在用这个 Agent、有哪些任务、哪些需要关注 | `trace_status`、风险等级、证据、成功/失败原始术语 |
| 用户任务列表 | 哪些任务需要我处理 | 成功/失败原始术语、证据细节 |
| 任务详情页 | 这条 Trace 到底发生了什么 | 无（展示全部结论和证据） |

### 9.2 措辞修正

`action_state=resolved` 统一显示“已完成”容易被理解为结果已确认。F01 确认 `incomplete` 只允许搭配 `none`、`low` 之后，这条风险已经消除：缺终止事件的 Trace 全部落入 `unconfirmed`，显示“待确认”，不会以“已完成”出现。因此：

- `resolved` 只包含有明确成功终止事件的 Trace，显示“已完成”。
- `unconfirmed` 显示“待确认”，对应缺终止事件且未判定风险的 Trace，需要用户自行确认结果。
- 原设计中“已完成但有异常 · 链路缺终止事件”的次级文案不再有合法来源，V1.1 不再使用。

### 9.3 Agent 页定位效率

Agent 页分组行在用户数量较多时需要保留定位能力，V1.1 要求：

- 分组标题旁提供搜索框，按 `requester_id` 或展示名过滤。
- 默认展开第一个分组，同时提供"全部展开/全部折叠"入口。
- 分组超过 20 个时默认只渲染前 20 个并提供"加载更多"，避免首屏过长。
...
Dashboard 采用“Agent → 发起用户 → Trace 任务 → 任务详情”的层级，避免同一 Agent 被多个用户调用时出现同名任务混淆。

| 页面 | 展示范围 | 核心内容 |
| --- | --- | --- |
| 首页 | Agent 列表 | Agent 名称/ID、最近活动、进入 Agent 页；本版本保持现有结构，不增加新的导航入口和状态角标 |
| Agent 页 | 当前 Agent 下按 `requester_id` 聚合的用户分组 | 发起用户、该用户的任务数量、最近任务时间；点击用户后查看其 Trace 任务 |
| 用户任务列表 | 当前 Agent + 当前发起用户 | 按 `trace_id` 区分每次任务；展示时间、原始请求摘要、`action_state` 行动状态 |
| 任务详情页 | 单个 Trace | 具体审计结论、链路状态、风险等级、是否人工介入、请求/目的/结果、证据链和原始日志 |

Agent 页不能只按 Agent 平铺任务，也不能把 `user_id` 当作任务发起人。用户分组键是 `requester_id`；缺失该字段的历史任务统一进入“发起人未知”分组，并标记任务上下文不完整。

Agent 页默认就地展开第一个用户分组，展开态有独立可分享 URL；不采用左侧常驻用户列表的双栏布局，避免窄屏降级。用户分组行展示发起用户、任务数量和最近任务时间。

### 12.5.1 视觉方向

Dashboard 采用暗色工程台风格：深色底、低饱和分隔线、等宽字体仅用于时间、计数、状态码和证据区。不采用高冲击力的先锋排版，因为该界面的首要读者是查看任务结论的非开发用户。

### 12.5.2 用户任务列表的展示语义

用户任务列表不出现“成功”“失败”“高风险”“中风险”“低风险”等原始术语，也不显示 `trace_status` 原文。列表第三列展示 6.5 定义的 `action_state` 派生文案，共五个取值：未封存为“审查中”，已封存但尚未出结论为“未审查”，已出结论为“需要介入”“待确认”或“已完成”。

- “审查中”对应 `in_progress`，即 `sealed_at` 为空，数据尚未收齐。
- “未审查”对应 `unreviewed`，即已封存但 `review_version=0`。历史回填任务全部落在该状态，它们不参与风险统计，也不代表无风险。
- “需要介入”对应 `needs_attention`，即 `risk_level=high`。
- “待确认”对应 `unconfirmed`，即缺终止事件且未被判定为风险项（`none`、`low`）的任务，需要用户自行确认结果。由于 `incomplete` 不允许搭配 `medium`、`high`，不存在“已被判定为风险的中断任务”出现在该状态的情况。等待用户超时未回复的任务也落在这里。
- “已完成”对应 `resolved`，即日志中已出现明确成功终止事件的任务，包含正常完成、低风险和中风险。

列表表头同时给出“N 条需要介入”的计数，让用户进入该用户后第一眼知道是否有必须处理的任务。风险等级、`trace_status`、审查批次等原始结论只在任务详情页展示。

### 12.5.3 任务详情页

任务详情页按以下顺序展示：

1. 任务结论：成功/失败、`trace_status`、`risk_level`、是否需要人工介入。
2. 任务上下文：发起用户、用户原始请求、Agent 预期目的、Agent 执行结果。
3. Agent 审计结果：审计结论、风险原因、审查批次和审计时间。
4. 证据链：Run、Agent、工具调用、错误、重试、循环和终止事件的时间顺序。
5. 原始日志：默认折叠，展开后查看完整 JSON、事件 ID、时间、事件、工具和状态。

原始日志作为证据保留，但默认折叠；Agent 审计结果和证据链在首屏可见。日报继续作为独立全局入口，不嵌入用户任务列表；日报中的任务可链接回 Agent、发起用户分组和具体 Trace。

### 12.5.4 页面线框与视觉稿

本节给出 V1.1 前端的信息架构与视觉基线，线框源码见 12.5.5。实现时以本节截图和源码共同为准。
...
源码是**线框参考**，用于固定信息架构、字段语义、文案和视觉方向；它不是可直接上线的实现，正式前端需要在项目现有 Dashboard 渲染链路（`src/auditReview/dashboardTemplate.js` 及对应路由）中按同样的信息层级实现，并复用现有鉴权、转义和组件渲染函数。

重新生成截图时在该目录执行 `pwsh ./render.ps1`，脚本用 Chrome 无头模式渲染并覆盖同级四张 PNG。修改视觉细节时改源码后重跑脚本，同时更新 12.5.4 的截图。
...
- 第 5 组 → `unconfirmed`，显示“待确认”。
- 第 6 组 → `needs_attention`，显示“需要介入”。
- 未封存（`sealed_at IS NULL`）→ `in_progress`，显示“审查中”，不参与告警统计。
...
- `incomplete + low` 必须派生为 `unconfirmed`，显示“待确认”，不得因为风险等级为 `low` 就进入 `resolved`。

额外覆盖状态机边界：

- `run.waiting_user` 后无 `run.resume`：在 `waitingUserTimeoutMinutes` 之前不封存，`action_state` 为 `in_progress`（“审查中”）；超时后封存（`sealed_reason='waiting_user_timeout'`），按 6.3.1 第 6 行判 `incomplete + none`，`action_state` 为 `unconfirmed`（“待确认”），**不得判为 `high`、不得发送飞书告警**。
...
- 首页不出现风险等级、工具、Finding、状态码。
- Agent 页分组键为 `requester_id`，缺失时进入“发起人未知”分组并标记上下文不完整；搜索、全部展开/折叠、超过 20 个分组的加载更多均可用。
- 用户任务列表状态文案固定为“审查中”“未审查”“需要介入”“待确认”“已完成”五个值，不出现成功/失败等原始术语，也不出现已废弃的“已完成但有异常”。
- 任务详情页五段顺序固定，原始日志默认折叠。
- 飞书卡片直达 `/dashboard/agents/{agent_id}/traces/{trace_id}`，首屏可见“需要人工介入”；两个路径参数均按 URL 规则编码。该 URL 由 `src/auditReview/visualization.js` 的 `dashboardUrlFor` 生成。
- 全站无表情符号；图标为 Lucide；720px 及以下宽度无横向滚动，`.kv` 与列表行纵向堆叠。
